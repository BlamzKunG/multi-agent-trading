...
{{
  ...
  "lot": float,
  "entry": float, // ระดับราคาจุดตั้ง Pending Order (Limit/Stop) ที่ได้เปรียบ ห้ามใส่ null เป็นอันขาด เพื่อป้องกันราคาเสียเปรียบ:
...
กติกาสำคัญในการตั้ง Pending Order:
1. ปฏิบัติตามมติของ Market Regime เสมอ (Regime ปัจจุบัน: {regime_report.get('regime') if regime_report else 'Uncertain'})
2. **ห้ามตั้งราคาเข้า Pending ใกล้ราคาตลาดปัจจุบันเกินไปเพื่อต้องการให้ออเดอร์แมตช์ง่ายเด็ดขาด** ราคาเข้าต้องเป็นระดับแนวรับ/แนวต้านที่มีนัยสำคัญทางโครงสร้างราคาจริงๆ เช่น สัมผัสเส้น EMA 50/200 หรือขอบล่างของสวิงโลว์เดิม (สำหรับ BUY LIMIT) หากราคาไม่วิ่งมาทดสอบตามเป้าหมาย ให้ยอมข้ามรอบและปล่อยให้ออเดอร์ยกเลิกไปเอง!
3. ตรวจสอบให้มั่นใจว่าราคาเข้าตั้งรอนั้นไม่ได้อยู่ใกล้ราคาตลาดปัจจุบันมากเกินไปจนเสี่ยงต่อการโดนปฏิเสธจากโบรกเกอร์ (ต้องห่างพอสมควรและตั้งอยู่บนระดับเทคนิคัลที่มีนัยสำคัญและได้เปรียบจริง)
4. หากมีคำสั่งล่วงหน้า (Pending Order) เปิดค้างอยู่ของกลยุทธ์นี้ (ข้อมูลด้านล่าง) คุณสามารถเลือก action เป็น HOLD, MODIFY, CANCEL, CANCEL_AND_NEW
5. ล็อตสูงสุดจำกัดที่ {max_allowed_lot} ล็อตที่คำนวณมาตามความเสี่ยง 1% คือ {final_lot}
6. ระยะ TP แนะนำห่าง 1.5 - 2 เท่าของระยะ SL แนะนำ (SL แนะนำห่างประมาณ: {sl_distance_usd:.2f} USD)"""
...
{{
  ...
  "lot": float,
  "entry": float, // ระดับราคาจุดตั้ง Pending Order (Limit/Stop) ที่ได้เปรียบ ห้ามใส่ null เป็นอันขาด เพื่อป้องกันราคาเสียเปรียบและลดความเสี่ยง
                            // - ช้อนซื้อ (Buy Limit) เมื่อราคาย่อตัวหาแนวรับย่อยสวิงต่ำ M15/H1
                            // - ขายดักบน (Sell Limit) เมื่อราคาขึ้นทดสอบแนวต้านย่อย M15/H1
                            // - ดักทะลุกรอบ (Buy/Sell Stop) เมื่อราคาทำ Breakout ข้ามผ่านแนวนัยสำคัญ
                            // - กรุณาใส่ตัวเลขราคา หรือระบุ null หากวิเคราะห์แล้วว่าราคาปัจจุบันได้เปรียบสุดขีดและต้องเปิดทันทีด้วย Market Price จริงๆ (ไม่แนะนำ)
  "sl": float_หรือ_null,
  "tp": float_หรือ_null,
  "reasoning": "ประโยคสรุปเหตุผลการตัดสินใจและการคำนวณจุดตั้งรับ/จุดตัดขาดทุนที่ได้เปรียบสไตล์ Day Trade"
}}

กติกาสำคัญในการตั้ง Pending Order:
1. ปฏิบัติตามมติของ Market Regime เสมอ (Regime ปัจจุบัน: {regime_report.get('regime') if regime_report else 'Uncertain'})
2. **ระดับราคาเข้า (entry) ต้องเลือกตั้งรอ ณ จุดที่ได้เปรียบทางโครงสร้างทางเทคนิคัลที่มีนัยสำคัญจริงๆ เท่านั้น** (เช่น แนวรับต้าน H1/H4 หรือขอบเส้นกรอบราคา) ห้ามตั้งราคาแบบขอไปทีใกล้ราคาตลาดปัจจุบัน หากราคาวิ่งไปไกลแล้วและไม่มีระดับเป้าหมายย้อนกลับ ให้สั่ง HOLD เพื่อเฝ้าตลาดต่อไป
3. ตรวจสอบให้มั่นใจว่าราคาเข้าตั้งรอนั้นตั้งอยู่บนแนวรับแนวต้านที่มีนัยสำคัญและได้เปรียบจริง ไม่ใช่ราคาที่ใกล้ราคาตลาดปัจจุบันมากเกินไปจนขาดความคุ้มค่าหรือเสี่ยงโดนโบรกเกอร์ปฏิเสธคำสั่ง (Rejected)
4. ตั้งระดับ SL และ TP อย่างมีเหตุผลตามสวิงราคา และให้ได้ระยะ R:R อย่างน้อย 1.5 เท่าขึ้นไป
5. หากมีคำสั่งล่วงหน้า (Pending Order) เปิดค้างอยู่ (ข้อมูลด้านล่าง) คุณสามารถเลือก action เป็น:
...
5. ล็อตสูงสุดจำกัดที่ {max_allowed_lot} ล็อตที่คำนวณตามความเสี่ยง 1% คือ {final_lot}
6. ออเดอร์ทั้งหมดต้องไม่ถือครองข้ามคืน"""
...
{{
  ...
  "lot": float,
  "entry": float, // ระดับราคาจุดตั้ง Pending Order (Limit/Stop) ที่ได้เปรียบ ห้ามใส่ null เป็นอันขาด เพื่อป้องกันราคาเสียเปรียบและลดความเสี่ยง
                            // - ช้อนซื้อระยะยาว (Buy Limit) ณ แนวรับสำคัญระดับโครงสร้าง H4/D1 หรือจุดแนวรับจิตวิทยาหนาๆ
                            // - ขายข้ามสัปดาห์ (Sell Limit) ณ แนวต้านหลักระดับโครงสร้าง H4/D1
                            // - ห้ามส่ง Market Order เป็นอันขาด เว้นแต่มีจังหวะขยับทะลุกรอบสวิงใหญ่ที่รุนแรงและได้เปรียบสุดขีดเท่านั้น
  "sl": float_หรือ_null,
  "tp": float_หรือ_null,
  "reasoning": "ประโยคสรุปแผนการเทรดสวิงระยะยาว การคำนวณสัดส่วน R:R ที่สูงเชิงแนวรับต้านยุทธศาสตร์ระดับ Macro"
}}

กติกาสำคัญในการตั้ง Pending Order:
1. ปฏิบัติตามมติของ Market Regime เสมอ (Regime ปัจจุบัน: {regime_report.get('regime') if regime_report else 'Uncertain'})
2. **ห้ามเข้าซื้อขายที่จุดสุ่มเสี่ยงและต้องวางคำสั่งล่วงหน้าในแนวยุทธศาสตร์ที่เกิดเปรียบสุดขีดเท่านั้น** (เช่น แนวรับหนา D1/W1 หรือโซนแนวสวิงหลักที่ชัดเจน) การตั้งราคาใกล้จุดปัจจุบันที่ไม่มีแนวรับรับรองจะทำให้ระบบเสียหาย หากราคาวิ่งเลยจุดไปแล้วให้ยินดีข้ามรอบ (HOLD)
3. ระดับราคาเข้าต้องอยู่บนแนวรับต้านหลักหรือสวิงหลักระดับโครงสร้างใหญ่ H4/D1/W1 เท่านั้น และต้องไม่ใกล้ราคาปัจจุบันมากเกินไปจนเสี่ยงต่อการถูกปฏิเสธคำสั่งซื้อขายล่วงหน้า (Rejected) หรือเสียเปรียบราคาในโซนผันผวน
4. การตั้ง SL และ TP: วาง SL ใต้แนวรับสวิงหรือเหนือแนวต้านสวิงที่มีนัยสำคัญ และ TP ควรได้สัดส่วน R:R 1.5 ถึง 3 เท่าขึ้นไป
5. หากมีคำสั่งล่วงหน้า (Pending Order) เปิดค้างอยู่ (ข้อมูลด้านล่าง) คุณสามารถเลือก action เป็น:
   - "HOLD": ถือคำสั่งล่วงหน้านี้ต่อตามเดิม
   - "MODIFY": แก้ไขราคาเข้า (entry), SL หรือ TP ของตั๋วใบนี้ (ระบุเลข ticket ให้ถูกต้อง)
   - "CANCEL": ยกเลิกคำสั่งล่วงหน้านี้ออกไปก่อนชั่วคราว
   - "CANCEL_AND_NEW": ยกเลิกคำสั่งเดิมแล้วต้องการยื่นตั้งคำสั่งล่วงหน้าอันใหม่ทันที
5. ล็อตสูงสุดจำกัดที่ {max_allowed_lot} ล็อตที่คำนวณตามความเสี่ยง 1% คือ {final_lot}
6. ยินดีถือออเดอร์ข้ามคืนได้หากสอดคล้องความเสี่ยงและแนวโน้มเชิงมหภาค"""
...
หน้าที่ของคุณคือการวิเคราะห์ราคาปัจจุบัน ประวัติแท่งเทียนล่าสุด และตัดสินใจส่งคำสั่งซื้อขายตามความได้เปรียบของสภาวะตลาด
คุณมีอิสระในการวิเคราะห์ 100% ไม่จำเป็นต้องกังวลเรื่องการกรองหรือกฎของอินดิเคเตอร์ใดๆ และคุณสามารถเลือกประเภทการเปิดออเดอร์เองได้

คุณต้องตอบกลับผลการวิเคราะห์และแผนตัดสินใจในรูปแบบ JSON โครงสร้างนี้เท่านั้น:
{{
  "action": "BUY" | "SELL" | "HOLD" | "MODIFY" | "CANCEL" | "CANCEL_AND_NEW",
  "ticket": int_หรือ_string_หรือ_null, // ใส่ Ticket ID หากต้องการจัดการคำสั่งล่วงหน้าเดิมที่มีอยู่
  "confidence": int, // น้ำหนักความมั่นใจในการวิเคราะห์ครั้งนี้เป็นเปอร์เซ็นต์ (0 ถึง 100)
  "entry_type": "MARKET" | "PENDING", // เลือกประเภทออเดอร์: MARKET (เข้าทันทีที่ราคาปัจจุบัน) หรือ PENDING (ตั้งรอราคาที่ต้องการ)
  "entry": float, // ราคาเข้าที่เสนอ (หากเป็น PENDING ให้ระบุราคาตั้งรอที่ได้เปรียบ | หากเป็น MARKET ให้ใส่ราคาตลาด {current_price:.2f})
  "hold_minutes": 15 | 30 | 45 | 60, // กรณีตอบ HOLD หรือต้องการหน่วงเวลา รอแท่งถัดไป ให้เลือกระบุช่วงเวลาพัก
  "reasoning": "คำอธิบายเชิงกลยุทธ์สั้นๆ สรุปแผนการวิเคราะห์ทางเทคนิคัลเฉพาะตัวของคุณ"
}}

ข้อมูลและการจัดการความเสี่ยง (Risk Management):
1. ระยะ SL และ TP ของคุณจะถูกคำนวณอัตโนมัติจากราคาเข้าโดยใช้ความผันผวนล่าสุด: SL = ATR * 2 | TP = ATR * 3
2. เมื่อคุณประเมินราคาเข้า:
   - หากเลือก "MARKET": ออเดอร์จะถูกเปิดทันทีที่ราคาตลาดปัจจุบัน
   - หากเลือก "PENDING": ให้ระบุระดับราคาเข้าที่ได้เปรียบเชิงโครงสร้างทางเทคนิคัลที่มีนัยสำคัญจริงๆ (เช่น แนวรับต้าน, โซนปฏิเสธราคา) และระดับต้องไม่ใกล้ราคาปัจจุบันมากเกินไปจนโดน Rejected
3. สำหรับคำสั่งล่วงหน้า (Pending Order) เก่า: คุณสามารถตัดสินใจได้อิสระว่าต้องการถือต่อ (HOLD), เลื่อนราคา (MODIFY), หรือยกเลิกคำสั่งเดิม (CANCEL)"""
