...
{{
  ...
  "lot": float,
  "entry": float_หรือ_null, // เน้นกรอกราคาเพื่อตั้งคำสั่งล่วงหน้า (Pending Order) ณ จุดเข้าที่ได้เปรียบจริง (Advantageous Entry) เพื่อเลี่ยงความเสียเปรียบและลดความเสี่ยง
...
กติกา:
1. ปฏิบัติตามมติของ Market Regime เสมอ (Regime ปัจจุบัน: {regime_report.get('regime') if regime_report else 'Uncertain'})
2. เน้นการใช้คำสั่งล่วงหน้า (Pending Order) เพื่อให้ได้จุดเข้าที่ได้เปรียบอย่างเด่นชัด หลีกเลี่ยง Market Order ยกเว้นจำเป็นจริงๆ
3. ตั้งระดับ SL และ TP อย่างมีเหตุผลตามสวิงราคา และให้ได้ระยะ R:R อย่างน้อย 1.5 เท่าขึ้นไป
4. หากมีคำสั่งล่วงหน้า (Pending Order) เปิดค้างอยู่ (ข้อมูลด้านล่าง) คุณสามารถเลือก action เป็น:
...
{{
  ...
  "lot": float,
  "entry": float_หรือ_null, // บังคับเน้นกรอกราคาเพื่อตั้งคำสั่งล่วงหน้า (Pending Order) ณ แนวสวิงที่คุ้มค่าสูงสุดเพื่อตัดปัญหา Slippage และ Gap Risk
...
กติกา:
1. ปฏิบัติตามมติของ Market Regime เสมอ (Regime ปัจจุบัน: {regime_report.get('regime') if regime_report else 'Uncertain'})
2. เน้นการใช้คำสั่งล่วงหน้า (Pending Order) ณ ระดับโครงสร้างราคาสำคัญเพื่อความได้เปรียบระดับโครงสร้างใหญ่ H4/D1
3. การตั้ง SL และ TP: วาง SL ใต้แนวรับสวิงหรือเหนือแนวต้านสวิงที่มีนัยสำคัญ และ TP ควรได้สัดส่วน R:R 1.5 ถึง 3 เท่าขึ้นไป
4. หากมีคำสั่งล่วงหน้า (Pending Order) เปิดค้างอยู่ (ข้อมูลด้านล่าง) คุณสามารถเลือก action เป็น:
   - "HOLD": ถือคำสั่งล่วงหน้านี้ต่อตามเดิม
   - "MODIFY": แก้ไขราคาเข้า (entry), SL หรือ TP ของตั๋วใบนี้ (ระบุเลข ticket ให้ถูกต้อง)
   - "CANCEL": ยกเลิกคำสั่งล่วงหน้านี้ออกไปก่อนชั่วคราว
